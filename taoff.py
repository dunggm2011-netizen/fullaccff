...
WEBHOOK_URL = os.environ.get("WEBHOOK_URL", "https://fullaccff.onrender.com/webhook")
...
def get_back_menu():
    ...
    keyboard.add(types.InlineKeyboardButton("⬅️ Quay lại Menu", callback_data='menu'))
    return keyboard
...
🌍 Region: `{region}`
{'👻 Ghost Mode' if is_ghost else ''}
📌 Nhập *số lượng acc* cần tạo (1-500):
""", parse_mode='Markdown', reply_markup=get_back_menu())
        ...
        if chat_id not in user_sessions:
            bot.send_message(chat_id, "❌ Session hết hạn! Bấm /start lại.")
            return
        ...
        show_saved_accounts(chat_id)

    elif data == 'rare':
        show_rare_accounts(chat_id)

    elif data == 'couples':
        show_couples_accounts(chat_id)

    elif data == 'download':
        show_download_menu(chat_id)

    elif data.startswith('dl_'):
        fname = data.replace('dl_', '')
...
def show_regions(message):
    ...
    keyboard.add(types.InlineKeyboardButton("⬅️ Menu", callback_data='menu'))
...
def process_count(message):
    ...
    try:
        ...
        msg = bot.send_message(chat_id, "❌ Nhập số từ 1-500:", reply_markup=get_back_menu())
        bot.register_next_step_handler(msg, process_count)
        return
    ...
    msg = bot.send_message(chat_id, f"✅ Số lượng: `{count}`\n\n👤 Nhập *prefix tên acc* (VD: Duy):", parse_mode='Markdown', reply_markup=get_back_menu())
...
def process_name(message):
    ...
    if not name or len(name) > 20:
        msg = bot.send_message(chat_id, "❌ Prefix không hợp lệ (1-20 ký tự):", reply_markup=get_back_menu())
        bot.register_next_step_handler(msg, process_name)
        return
    ...
    msg = bot.send_message(chat_id, f"✅ Name prefix: `{name}`\n\n🔑 Nhập *prefix password* (VD: Duy):", parse_mode='Markdown', reply_markup=get_back_menu())
...
def process_password(message):
    ...
    msg = bot.send_message(chat_id, f"✅ Password prefix: `{pwd}`\n\n🧵 Nhập *số threads* (1-5, khuyến nghị 2-3):", parse_mode='Markdown', reply_markup=get_back_menu())
...
def process_threads(message):
    ...
    try:
        ...
        msg = bot.send_message(chat_id, "❌ Nhập số từ 1-5:", reply_markup=get_back_menu())
        bot.register_next_step_handler(msg, process_threads)
        return
...
def show_saved_accounts(chat_id):
    files = []
    ...
    bot.send_message(chat_id, text, parse_mode='Markdown', reply_markup=get_back_menu())

def show_rare_accounts(chat_id):
    files = []
...
def show_couples_accounts(chat_id):
    files = []
